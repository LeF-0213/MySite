...
개인 포트폴리오 및 블로그 웹사이트로 기술 프로젝트 소개, 기술 스택, 그리고 연락처 정보를 한눈에 볼 수 있다.

## 🎨 테마 & 애니메이션

Dark와 Light 두 가지 테마를 지원하며, 클릭과 스크롤 시 다양한 인터랙티브 효과를 적용
...
1. **테마 전환**: 상단의 테마 토글 버튼을 클릭하여 Dark/Light 테마를 전환할 수 있다.
2. **인터랙션**: 각 요소를 클릭하면 부드러운 애니메이션 효과가 작동.
3. **스크롤 효과**: 페이지를 스크롤하면 화살표가 사라지고 홈 영역이 옅어지는 등 동적인 효과
4. **프로젝트 관리**: 프로젝트 폴더의 개수가 동적으로 변화
